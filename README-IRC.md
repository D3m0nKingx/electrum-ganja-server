IRC is used by Electrum Ganja server to find 'peers' - other Electrum servers. The
current list can be seen by running:

  electrum-ganja-server peers
...
mode, `electrum-ganja-server peers` will always return an empty list.
...
composing the IRC nickname to identify your server on #electrum-ganja.
